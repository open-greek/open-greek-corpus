...
THIRD_SHEET = DATA / "nonfinal_grave_tranche_thirdparty.json"
...
# The sources cisco's 2026-08-10 decision covered when he set the bar for
# correcting other people's editions. Fixed rather than derived, so a source
# added to the corpus later cannot slide into the tranche without its own
# decision; byzantine_vernacular is deliberately absent.
THIRD_PARTY_SOURCES = ("byzantium_gr", "cgpg", "dfhg", "first1k", "perseus",
                       "pta", "saws", "wikisource")
...
# Other people's editions go stricter still, and the class itself must be big,
# not just the winning reading's hits.
THIRDPARTY_SHARE, THIRDPARTY_CLASS = 0.99, 100
...
def main() -> None:
    ...
    third_party: dict[str, collections.Counter] = {}
        ...
        for line in fp.read_text(encoding="utf-8").splitlines():
            ...
            for w in _GK.findall(r.get("text") or ""):
                ...
                if has_nonfinal_grave(w):
                    ...
                    if src in THIRD_PARTY_SOURCES:
                        third_party.setdefault(
                            w, collections.Counter())[src] += 1
    ...
    for form, n in forms.most_common():
        ...
        if (accent_only and share >= TRANCHE_SHARE and hits >= TRANCHE_CLEAN
                and ocr_tokens[form]):
            tranche.append(rec)
        # Gated here on the RAW share and class size, not the rounded rec, and
        # on the FULL decided list, not the artifact's 400-row sample: the sheet
        # applied on 2026-08-10 was generated by an uncommitted one-off that
        # read the sample, so every eligible form below the window (ὃροι at
        # share 1.0 among them) silently missed the pass.
        if (accent_only and share >= THIRDPARTY_SHARE
                and size >= THIRDPARTY_CLASS and form in third_party):
            srcs = third_party[form]
            thirdparty.append({
                "form": form, "target": target, "share": round(share, 4),
                "clean_class": size, "clean_hits": hits,
                "accent_only": accent_only, "runner_up": rec["runner_up"],
                "tokens_in_third_party": sum(srcs.values()),
                "by_source": {s: srcs[s] for s in sorted(srcs)}})
    ...
    OUT.write_text(json.dumps({
        ...
        "skeleton_class": {
            ...
            "decided_forms_total": len(decided),
            "decided_is_a_sample": "the 400 largest by flagged tokens, of "
                                   "decided_forms_total. The tranche sheets are "
                                   "emitted from the full list; a sheet built "
                                   "from this sample truncates the pass, which "
                                   "is how the 2026-08-10 third-party apply "
                                   "missed 418 eligible forms.",
            "decided": decided[:400],
        },
    ...
    # The third-party tranche: the same class inside text this repo republishes
    # from other people's editions. Emitted from the FULL decided list (the gate
    # sits in the loop above); what is applied is recorded under
    # data/corpus_changes/, never here.
    tp_tok = sum(r["tokens_in_third_party"] for r in thirdparty)
    tp_w = (sum(r["tokens_in_third_party"] * r["share"] for r in thirdparty)
            / tp_tok) if tp_tok else 0
    THIRD_SHEET.write_text(json.dumps({
        "what": "the stricter tranche for text this repo republishes from "
                "other editions",
        "issue": "open-greek/open-greek-corpus#31",
        "status": "rows here clear the recorded bar and are NOT applied; each "
                  "apply is recorded under data/corpus_changes/. Regenerated "
                  "from the FULL decided class by measure_nonfinal_graves.py "
                  "--write: the sheet applied on 2026-08-10 came from an "
                  "uncommitted one-off reading the artifact's 400-row decided "
                  "sample, which silently truncated the pass.",
        "decision": "cisco, 2026-08-10: we may correct other people's editions "
                    "where we are SURE they are wrong. This is the bar set for "
                    "'sure' and the number it earns.",
        "rule": "share >= 0.99 of a skeleton class holding >= 100 clean "
                "non-OCR tokens, and accent-only. Our own OCR went at >= 0.95; "
                "this is an order of magnitude stricter in expected error, "
                "because a wrong correction here is our error inside someone "
                "else's edition.",
        "sources": list(THIRD_PARTY_SOURCES),
        "forms": len(thirdparty), "tokens": tp_tok,
        "token_weighted_share": round(tp_w, 5),
        "expected_wrong_tokens": round(tp_tok * (1 - tp_w)),
        "not_touched": "the other flagged tokens in these sources sit below "
                       "the bar and stay wrong and recorded rather than "
                       "guessed at.",
        "rows": thirdparty,
    }, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
    print(f"    third-party tranche (not applied): {len(thirdparty)} forms, "
          f"{tp_tok:,} tokens, weighted {tp_w:.4f}")
    ...
    print(f"\nwrote {OUT.relative_to(REPO)}, {SHEET.relative_to(REPO)}, "
          f"{MARK_SHEET.relative_to(REPO)} and {THIRD_SHEET.relative_to(REPO)}")
